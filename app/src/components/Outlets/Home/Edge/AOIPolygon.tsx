...
const AOIPolygon = ({
    ...
    const handleDrag = (xDiffPct: number, yDiffPct: number) => {
        ...
        let newTopPosition = AOIAsPcts[selectedPointIndex].top + yDiffPct;
        let newLeftPosition = AOIAsPcts[selectedPointIndex].left + xDiffPct;
        ...
        const newPointPositions = [...AOIAsPcts];
        ...
        setAOIMidPointsAsPcts(newMidPoints);
        setAOIAsPcts(newPointPositions);
    };
    ...
    const setNewPoints = () => {
        const newPoints = AOIAsPcts.map((point) => {
    ...
    const handleMidPointMouseDown = (
        ...
        e.stopPropagation();
        setMouseStartingPos({ x: e.clientX, y: e.clientY });
        const scaleLineAsPctsCopy = [...AOIAsPcts];
        const newScalePoint = AOIMidPointsAsPcts[midPointIndex];
        scaleLineAsPctsCopy.splice(midPointIndex + 1, 0, newScalePoint);

        setAOIAsPcts(scaleLineAsPctsCopy);
    ...
    const handleRightClick = (
        ...
        if (AOI.length > 4 && AOIAsPcts.length > 4) {
            const newScaleLine = [...AOI];
            const newScaleLineAsPcts = [...AOIAsPcts];
    ...
    return (
        <>
            ...
            {AOIMidPointsAsPcts.map((point, index) => {
                return (
                    <div
                        style={determinePointStyles(point)}
