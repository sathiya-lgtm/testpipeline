...
// API Calls
import fetchSPAuditReport from '../../../api_calls/fetchSPAuditReport';
...
import generateSPAuditReport, {
    addSPAuditReportHeaders,
} from '../../../utils/generateSPAuditReport';
...
const SPAuditReport: FC<IProps> = ({
    ...
    const [spAuditReportCSV, setSpAuditReportCSV] = useState<string>('');
    ...
    const spAuditReportMutation = useMutation({
        mutationFn: fetchSPAuditReport,
        onError: (err: any) => {
    ...
    const getReport = async (e: FormEvent) => {
        ...
        if (diffDays === 1) {
            ...
            const results = await spAuditReportMutation.mutateAsync({
                user: activeUser,
                spAuditReportData,
            });

            csvString = addSPAuditReportHeaders(results.headers);
            csvString = generateSPAuditReport(results, csvString);
            ...
            if (spAuditReportCSV) {
                URL.revokeObjectURL(spAuditReportCSV);
            }
            ...
            setSpAuditReportCSV(url);
            setIsLoading(false);
            setReportStatus({
                daysGenerated: 0,
                totalDays: 0,
            });
            toast.success('SP Audit Report Generated');
        } else {
            for (let i = 0; i < diffDays; i += 1) {
                ...
                const serviceProviderAccountId =
                    activeUser.account_type === AccountType.Evolon
                        ? Number(selectedServiceProvider?.value ?? 0)
            ...
            {isLoading && (
                ...
                >
                    <div className="bulk-upload-text-container">
                        <h2>Generating SP Audit Report...</h2>
                        <h2>
            ...
            {spAuditReportCSV && (
                <div className="download-container">
                    ...
                    <a
                        ...
                        href={spAuditReportCSV}
                        download={`${serviceProviderName}-SP_AUDIT_REPORT`}
