 ...
 * @param {string} apiDateString - Date-time string from API in following format: {2023-03-29 21:30:11} (Note: the "|tz=UTC" suffice is removed prior).
 * @returns {string} Date in local time in following format {mm-dd-yyyy}
 */
export default (apiDateString: string): string => {
    ...
    const formattedDate: string = format(new Date(`${apiDateString}Z`), 'P');
